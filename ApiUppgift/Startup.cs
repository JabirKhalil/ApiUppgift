...
namespace ApiUppgift
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            //                                          var adminId = long.Parse(context.Principal.FindFirst("AdminId").Value);
            //                                          if (adminId <= 0)
            //                                          {
            ...
            .AddJwtBearer(jwt =>
            {
                jwt.Events = new JwtBearerEvents
                {
                    OnTokenValidated = context =>
                    {
                        var userId = long.Parse(context.Principal.FindFirst("AdminId").Value);
                        if (userId <= 0)
                            context.Fail("Unauthorized");
            ...
            {
                c.SwaggerDoc("v1", new OpenApiInfo { Title = "ApiUppgift", Version = "v1" });
            });
        }
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            if (env.IsDevelopment())
            {
                ...
                app.UseSwagger();
                app.UseSwaggerUI(c => c.SwaggerEndpoint("/swagger/v1/swagger.json", "ApiUppgift v1"));
            }
